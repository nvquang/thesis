...
class VAE(object):
    ...
    def __init__(self, network_architecture, data_prior, transfer_fct=tf.nn.softplus,
        ...
        ckpt = tf.train.get_checkpoint_state(self.save_file)
        if ckpt:
    ...
    def _prior_loss(self,prior, weights):
        ...
        self.prior_loss = 20.0*tf.reduce_sum((prior - self.x_bin*Wo)**2, [1, 2])
    ...
    def _create_loss_optimizer(self):
        ...
        if self.prior is not None:
            weight_recog = [self.network_weights["weights_recog"], self.network_weights["biases_recog"]]
    ...
    def topic_prop(self, X):
        ...
        """
        theta_ = self.sess.run(tf.nn.softmax(self.z_mean),feed_dict={self.x: X,self.keep_prob: 1.0})
        return theta_

    def save_model(self, epoch_numb):
        # save model
        self.saver.save(self.sess, self.save_file, global_step=epoch_numb)
